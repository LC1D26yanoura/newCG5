...
ID3D12Resource* CreateRenderTargetResource(ID3D12Device* device, int32_t width, uint32_t height, DXGI_FORMAT format, const FLOAT* clearColor);
ID3D12Resource* CreateRenderTextureResource(ID3D12Device* device, int32_t width, uint32_t height, DXGI_FORMAT format, const FLOAT* clearColor);
...
int WINAPI WinMain(_In_ HINSTANCE, _In_opt_ HINSTANCE, _In_ LPSTR, _In_ int) {
	...
	// エンジンの初期化
	KamataEngine::Initialize(L"LE3D_05_ヤノウラ_ハルト_CG5");
	...
	ID3D12Resource* renderTextureResource = CreateRenderTargetResource(device, WinApp::kWindowWidth, WinApp::kWindowHeight, DXGI_FORMAT_R8G8B8A8_UNORM_SRGB, kRenderTargetClearColor);
...
ID3D12Resource* CreateRenderTargetResource(ID3D12Device* device, int32_t width, uint32_t height, DXGI_FORMAT clearFormat, const FLOAT* clearColor) {
